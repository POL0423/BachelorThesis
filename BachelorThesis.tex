...
\CzechAbstract{
    ...
    motivace k~tvorbě tohoto projektu společně s~přehledem podobných projektů,
    dále pokračuji rozborem technologií používaných pro~návrh a~implementaci
    webových aplikací s~podrobným porovnáním jednotlivých knihoven
    a~frameworků, zhodnocením jejich výhod a~nevýhod a~určením jejich
    optimálních aplikací, následně řeším technický návrh struktury této
    konkrétní webové aplikace, přípravu prostředí a~samotnou implementaci
    a~vývoj aplikace a~práci zakončuji zhodnocením projektu.
}
